...
custom_layer = Dense(num_classes, activation='softmax', name='Customlayer')(last_layer)
...
model_path = "Models/VGG16-Custom4Layer.h5"
...
img_path = r'C:\Users\INSANAnD\Desktop\ML_BootCamp\Transfer learning\Transfer-Learning-in-keras---custom-data-master\TestImage\testImage.jpg'
img = image.load_img(img_path, target_size=(224, 224))
...
imageData = np.expand_dims(img_array, axis=0)
# prepare the image for the VGG model
imageData = preprocess_input(imageData)
# predict the probability across all output classes
prediction_with_topLayer = model.predict(imageData)
# get the max array index 
max_predicted_index = prediction_with_topLayer.argmax()
# get the max array index value
predictedPercentile = prediction_with_topLayer.max() * 100
# get the mapped label name 
switcher = { 
0: "Human", 
1: "horse", 
2: "dog", 
...
LabelIdentified = switcher.get(max_predicted_index, "Nothing Matched")         
print('Prediction : {} -- {}% ' .format(LabelIdentified, str(round(predictedPercentile, 2))))
